...
//   ۵. ← اضافه: حالت بسته/جمع‌شده شبیه نوار اکشن؛ کلیک فقط پلیر را باز می‌کند (بدون پخش خودکار)
//   ۶. ← اضافه: اگر هنگام باز کردن پلیر هنوز خواننده‌ای پیدا نشده (مثلاً به‌خاطر اینترنت ضعیف
//      در لحظه ورود به صفحه)، دوباره تلاش برای دریافت لیست خواننده‌ها و لود صدا انجام می‌شود.
//
...
class _AudioPlayerWidgetState extends State<AudioPlayerWidget> {
  ...
  /// با کلیک روی نوار بسته‌شده: پلیر باز می‌شود (بدون پخش خودکار).
  /// اگر تا این لحظه خواننده‌ای پیدا نشده (مثلاً اینترنت ضعیف هنگام ورود به صفحه)،
  /// یک تلاش دوباره برای دریافت لیست خواننده‌ها و لود صدا انجام می‌شود.
  void _expand() {
    setState(() => _isExpanded = true);
    widget.onExpansionChanged?.call(true); // ← اضافه
    final ctrl = widget.controller;
    final needsRetry =
        !ctrl.isLoadingRecitations &&
        ctrl.recitations.isEmpty &&
        !ctrl.isAudioLoaded;

    if (needsRetry) {
      _initialized = false; // ← اجازه می‌دهد _loadIfNeeded دوباره تلاش کند
      _loadIfNeeded();
    }
  }
